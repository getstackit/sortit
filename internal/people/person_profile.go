...
func (h GetPersonProfileHandler) Handle(ctx context.Context, person string, filter issueviews.IssueStatusFilter) (PersonTagProfile, error) {
	...
	if store, ok := h.Store.(peopleAnalyticsLister); ok {
		...
		return buildPersonTagProfile(items, person, issueviews.IssueStatusFilterAll, tagSpecificity), nil
	}
	...
	if store, ok := h.Store.(issueviews.FilteredIssueLister); ok {
		...
		return buildPersonTagProfile(peopleAnalyticsIssuesFromIssues(items), person, issueviews.IssueStatusFilterAll, tagSpecificity), nil
	}
...
func buildPersonTagProfile(allIssues []issues.PeopleAnalyticsIssue, person string, filter issueviews.IssueStatusFilter, tagSpecificity map[string]*float64) PersonTagProfile {
