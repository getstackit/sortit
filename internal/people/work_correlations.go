...
func (h WorkCorrelationsHandler) Handle(ctx context.Context, filter issueviews.IssueStatusFilter) (WorkCorrelationsResult, error) {
	...
	if store, ok := h.Store.(peopleAnalyticsLister); ok {
		items, err := store.ListPeopleAnalytics(ctx, issues.ListOptions{
			Status: issueviews.IssueStatusFromFilter(filter),
		})
		if err != nil {
			return WorkCorrelationsResult{}, err
		}
		return buildWorkCorrelations(items, issueviews.IssueStatusFilterAll, tagSpecificity), nil
	}
	...
	if store, ok := h.Store.(issueviews.FilteredIssueLister); ok {
		...
		return buildWorkCorrelations(peopleAnalyticsIssuesFromIssues(items), issueviews.IssueStatusFilterAll, tagSpecificity), nil
	}
...
func buildWorkCorrelations(allIssues []issues.PeopleAnalyticsIssue, filter issueviews.IssueStatusFilter, tagSpecificity map[string]*float64) WorkCorrelationsResult {
...
func filterPeopleAnalyticsByStatus(items []issues.PeopleAnalyticsIssue, filter issueviews.IssueStatusFilter) []issues.PeopleAnalyticsIssue {
	if filter == "" {
		filter = issueviews.IssueStatusFilterOpen
	}
	if filter == issueviews.IssueStatusFilterAll {
		return items
	}
	...
	for _, item := range items {
		...
		if filter == issueviews.IssueStatusFilterOpen && status == issues.StatusOpen {
			filtered = append(filtered, item)
		}
		if filter == issueviews.IssueStatusFilterClosed && status == issues.StatusClosed {
			filtered = append(filtered, item)
		}
	}
	return filtered
...
func loadTagSpecificityMap(ctx context.Context, catalog *tags.CatalogService) map[string]*float64 {
	if catalog == nil {
		return nil
	}
