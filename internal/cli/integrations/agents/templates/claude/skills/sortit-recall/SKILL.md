---
name: sortit-recall
description: Recall durable Sortit memories relevant to what you are about to do — prior decisions, constraints, lessons, patterns, and references. Use before deciding, implementing, or answering, the same way you search issues before creating one.
...
```bash
command sortit memory search "$ARGUMENTS"
```
...
3. If a recalled memory is outdated, do not silently ignore it — supersede or
   archive it via the **sortit-memory** skill (Skill tool, or `/sortit-memory`).
