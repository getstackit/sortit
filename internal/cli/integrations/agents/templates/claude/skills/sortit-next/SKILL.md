---
name: sortit-next
description: Find the current user's next Sortit issue — assigned work first, then open issues matched to their tag profile when nothing is assigned. Use for a starter issue, next task, or personalized recommendation.
...
Current assigned work: !`command sortit mine`
...
After surfacing the recommended issue, use AskUserQuestion to offer: get full details (`command sortit issues get <id>`), explore adjacent work (`/sortit-explore <id>`), or assign it to the user (`/sortit-assign`).
