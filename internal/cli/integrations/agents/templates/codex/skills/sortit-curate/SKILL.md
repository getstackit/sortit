---
name: sortit-curate
description: Act as the Sortit librarian — sweep the corpus for curation candidates (duplicates, stale issues, enrichment health, quiet/redundant memories), judge them with codebase context, and file propose-only curation moves for a human to accept. Use to tidy the issue/memory graph, run a curation pass, or keep the library healthy. Trigger phrases include "run a curation sweep", "curate the issues", "find duplicate issues to merge", "clean up stale issues", "propose curation moves", "tidy the memory graph", "act as the librarian", and "keep the library healthy".
---
...
Use `$sortit-explore` to pull adjacent work around a candidate issue, and
`$sortit-diagnose` to inspect factor-model quality before proposing re-enrichment.
...
6. **One sweep at a time.** Pull, judge, propose, summarize. Loop on a cadence if
