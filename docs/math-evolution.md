...
| **Phase 4** | **Started, backend math slice only.** Deterministic NMF over positive ridge loadings now exists as an in-memory `internal/issuethemes` package: it accepts `F⁺` rows, emits issue theme weights `W`, theme tag loadings `H`, top tags, and normalized theme centroids. Still missing: persistence, refresh worker, API endpoints, theme list/detail UI, and wiring from live ridge decompositions into a stored theme projection. | `internal/issuethemes` |
