...
| **Phase 3** | **3a measured (shadow).** `ComputeRidgeDecomposition` builds per-issue `f_i`, reconstruction `Tᵀf`, residual, and true R² = 1 − ‖e − Tᵀf‖²/‖e‖²; `RidgeBlend` offers tag-space and reconstruction-space similarity. The matheval shadow harness (`TestRidgeShadowComparison`, `-ridge`) compares both against rank-1. **Result:** with the debug-endpoint default `λ_unscored=0.05` ridge *regresses* ranking (NDCG 0.867 vs 0.874) and its R² (~0.90) is overfit — unscored tags soak up variance under the weak penalty. Properly regularized (`λ_unscored≈1.0`, tag-space similarity) ridge reaches NDCG ~0.933. The ranking path therefore needs its own λ_unscored, separate from the drift-diagnostic endpoint's. 3b/3c blocked on calibrating that λ on held-out / real embeddings (the fixtures generate embeddings as relevance-weighted tag sums, so they can't fairly adjudicate). `factor_model.go` still powers production blending. | `internal/issuemath/ridge_decomposition.go`, `internal/matheval/ridge_shadow_test.go` |
