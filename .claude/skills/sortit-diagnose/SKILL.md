...
description: Diagnose factor-model quality and find issues where the tag taxonomy is failing. Use when the user wants to improve tags, understand R², or find issues that need re-enrichment.
...
```bash
command sortit debug issue-r2 "$ARGUMENTS"
```
...
- To see existing tags: `command sortit tags list` (or hand off to the **sortit-tags** skill — Skill tool, or `/sortit-tags`)
- To explore what an issue is about: `command sortit issues get <id>` (or hand off to the **sortit-get** skill — Skill tool, or `/sortit-get <id>`)
